...
function isFriend(member:Buddy) : boolean {
    return (member as Friend).age !== undefined;
}

function getBuddyListFriends(list: BuddyList): Friend[] {
 return list.members.reduce((friendsList : Friend[],member:Buddy) => {
    if(isFriend(member)) {
        friendsList.push(member as Friend);
    }
    return friendsList;
 },[]);
}
